...
func _ready() -> void:
	...
	tween.tween_property(drag_collision.shape, "radius", size/5, data.form_time)
	tween.tween_property(drag_collision.shape, "height", size, data.form_time)
	tween.tween_property(effect_collision.shape, "radius", size/5, data.form_time)
	tween.tween_property(effect_collision.shape, "height", size, data.form_time)
	tween.tween_property(visual, "scale", Vector2(0.25, 0.25) * size/200, data.form_time)
	await tween.finished
	
	lifetime_timer.wait_time = randf_range(data.min_lifetime, data.max_lifetime)
...
func _process(delta: float) -> void:
	...
	if !follow:
		...
		position += direction * min(data.speed * delta * data.speed_mult, (get_global_mouse_position() - global_position).length())
...
func _on_lifetime_timer_timeout() -> void:
	var tween = get_tree().create_tween()
	tween.set_parallel(true)
	tween.tween_property(drag_collision.shape, "radius", 0, data.dissolve_time)
	tween.tween_property(drag_collision.shape, "height", 0, data.dissolve_time)
	tween.tween_property(effect_collision.shape, "radius", 0, data.dissolve_time)
	tween.tween_property(effect_collision.shape, "height", 0, data.dissolve_time)
	tween.tween_property(visual, "scale", Vector2.ZERO, data.dissolve_time)
	await tween.finished
